...
cat <<\EOF > ${rootdir}/etc/keepalived/keepalived.conf
...
lxc.rootfs = /var/lib/lxc/lvs1/rootfs0
lxc.utsname = lvs1
...
ssh -i ${SSHKEY} root@${LXCHOST} "mkdir -p ${LXCHOSTDIR}/${NAME}/rootfs0"
rsync -e "ssh -i ${SSHKEY}" -Ha ${rootdir}/ root@${LXCHOST}:${LXCHOSTDIR}/${NAME}/rootfs0/
rsync -e "ssh -i ${SSHKEY}" -Ha config root@${LXCHOST}:${LXCHOSTDIR}/${NAME}/
